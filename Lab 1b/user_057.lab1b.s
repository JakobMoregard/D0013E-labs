...
save:		li	$t4, 0x7bd9f87d		#initialize 0x7bd9f87d
...
decode:		addiu	$sp, $sp, -4		# push the return addr...
		...
		not	$t0, $t0		#ONE'S-COMPLIMENT of codgen
...
then:		sb	$zero, 0($a1)		#store byte 0 in bytearr
		...
		bal	codgen			#calls codgen metho
		or	$t5, $v0, $zero

		not	$t5, $t5		#one's-compliment
...
exit2:		addiu	$v0, $t5, 0		#return r
		move 	$sp, $fp		# destroy all local param
      		lw    	$fp, 0($sp)		# restore old $fp
       		addiu 	$sp, $sp, 4		# and pop that word,
       		lw    	$31, 0($sp)		# restore return address,
       		addiu 	$sp, $sp, 4		# and pop that word.
